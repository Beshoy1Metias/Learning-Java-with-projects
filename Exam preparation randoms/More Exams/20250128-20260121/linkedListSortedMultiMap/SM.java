/**
   ...
   duplicate keys are allowed and the keys are bounded to classes that implement the
   ...
   @version 26 Dec. 2024
*/
// Implementation using a linked list. Asymptotic time complexity of the find method is O(n).
public class SM<K extends Comparable<K>,V> implements SortedMultimap<K, V>
{  
   // instance variables
   private ListNode head;       // head list node of linked list - tail is not necessary
   ...
   */
   ...
   // search for a mapping with specified key in this sorted multimap
   // k is the specified key
   // returns the reference to the list node before the list node where a mapping with the specified
   // key is or the tail node of the linked list if a mapping with the specified key in not in this map
   private ListNode search(K k) // O(n) in average and worst cases
   {
   ...
   */
   ...
   {     
      ...
      ListNode newHead = new ListNode(null, null, head);  // creates a new head for the linked list
      head = newHead;                     // sets new head
   ...
   /**
      Removes a mapping for the specified key from this sorted multimap if it is present.
