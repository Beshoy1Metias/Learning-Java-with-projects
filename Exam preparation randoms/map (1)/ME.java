...
public class ME <K extends Comparable, V extends Comparable> extends M <K, V>
{
   ...
   */
   public boolean containsValue(V value)
   {
	  ...
	  {
		  if(values[i].equals(value)){pos = i;}
	  }
	  return pos>=0;
   }
   ...
   */
   public Comparable[] valueSet()
   {
      Object[] values = values();
	  mergeSort(values);
	  Comparable[] valueSet = new Comparable[values.length];
	  int count =0;
	  valueSet[count++]=(Comparable)values[0];
	  for(int i=1; i<values.length; i++)
	  {
		  if(values[i-1].equals(values[i])){continue;}
		  valueSet[count++]=(Comparable)values[i];
	  }
	  Comparable[] output = new Comparable[count];
	  System.arraycopy(valueSet, 0, output, 0, count);
	  return output;
   }
